...
namespace InsuranceAPI.Tests
{
    public class InsuranceRepositoryTests
    {
        ...
        public async Task AddInsuranceAsync_ShouldReturnSuccess_WhenInsuranceIsNew()
        {
            ...
            var insurance = new Insurance { Id = Guid.NewGuid(), PersonalIdentificationNumber = "123", InsuredItem = "Car", InsuranceProductId = Guid.NewGuid() };
            _cacheMock.Setup(c => c.GetAsync<Insurance>(It.IsAny<string>())).ReturnsAsync(Result<Insurance?>.Failure("Not found"));
        ...
        public async Task GetInsurancesByPersonalIdentificationNumberAsync_ShouldReturnInsurances_WhenFoundInCache()
        {
            ...
            var repository = new InsuranceRepository(dbContext, _logger, _cacheMock.Object, _configMock.Object);
            var pin = "123";
            var insurances = new List<Insurance> { new Insurance { Id = Guid.NewGuid(), PersonalIdentificationNumber = pin, InsuredItem = "Car", InsuranceProductId = Guid.NewGuid() } };
            _cacheMock.Setup(c => c.GetAsync<IEnumerable<Insurance>>(It.IsAny<string>())).ReturnsAsync(Result<IEnumerable<Insurance>?>.Success(insurances));
